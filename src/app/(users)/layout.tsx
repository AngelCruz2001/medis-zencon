...
export default function FinalLayout({
  ...
  return (
    ...
    <div className="flex justify-between h-full max-w-full max-h-full overflow-hidden font-sans text-font">
      <nav
        className="w-[100px] md:w-[230px] transition-all shadow-light h-[100vh] py-3 overflow-x-hidden bg-secondary z-20 flex flex-col items-center justify-center shadow-xl">
      ...
      <div className="flex-1">
        <div
          className="flex items-center w-full h-16 px-14 py-2 bg-primary sticky between"
        >
          <p >
            {
              options.find(({ path }) => path === pathName)?.name
            }
          </p>
            <MdNotificationsNone className="w-auto h-8 text-secondary cursor-pointer"/>
        </div>

        <main id="app" className="overflow-y-auto h-full w-full bg-background">
